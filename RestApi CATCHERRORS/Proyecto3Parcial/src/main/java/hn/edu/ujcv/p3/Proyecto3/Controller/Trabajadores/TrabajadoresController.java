...
public class TrabajadoresController {
    ...
    public ResponseEntity<Object> addTrabajador (@RequestBody Trabajadores trabajador){
        try{
            if(trabajador.getNombre().isEmpty()){
                throw new BusinessException("Introduzca un trabajador");
            }
            if (trabajador.getNumerotelefono() == 0 ){
                throw new BusinessException("Introduzca un numero de telefono valido");
            }
            if (trabajador.getPuesto().isEmpty()){
                throw new BusinessException("El trabajador tiene que tener un puesto ");

            }
            if (trabajador.getFechainicio().isEmpty()){
