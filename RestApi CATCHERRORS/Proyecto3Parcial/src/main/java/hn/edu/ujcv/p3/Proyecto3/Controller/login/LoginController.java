...
public class LoginController {
    ...
    public ResponseEntity<Object> addLog(@RequestBody Login login) {
        try {
            if (login.getUsuario().isEmpty()){
                throw new BusinessException("El Usuario no puede estar vacio");
            }else{
                if (login.getClave().isEmpty()){
                    throw new BusinessException("La clave no puede estar vacia");
                }else{
                   if (login.getEmail().isEmpty()){
                       throw new BusinessException("Debe insertar un Email.");

                   }else
                       if (login.getNumerotelefono()==0){
                           throw new BusinessException("Debe insertar un numero de telefono valido");
                       }else
                           if (login.getFechanacimiento().isEmpty()){
                               throw new BusinessException("Introduzca una fecha valida.");
                           }
                }
